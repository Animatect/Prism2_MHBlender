...
from Prism_MHExtension_Products import Prism_MHExtension_Products
...
class Prism_MHExtension_Functions(object):
    def __init__(self, core, plugin):
        ...
        # Register callback for ProductBrowser (delegate to products manager)
        self.core.registerCallback("onProductBrowserOpen", self.productsManager.onProductBrowserOpen, plugin=self.plugin)
    ...
    def onPluginLoaded(self, plugin):
        if not self.blendFunctions:
            if self.core.appPlugin.appShortName.lower() == "bld":
                ...
                # Initialize USD export state module
                if not self.usdExportState:
                    sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(__file__)), "Integrations", "Blender"))
                    import MHUsdExportState
                    self.usdExportState = MHUsdExportState.MHUsdExportState(self.core)
                    logger.debug("Initialized USD Export State module")
...
class %s(QWidget, %s.%s, %s.%sClass):
    ...
    def onStateStartup(self, state):
        """Delegate to USD export state module"""
    ...
    def onStateGetSettings(self, state, settings):
        """Delegate to USD export state module"""
        if self.usdExportState:
            self.usdExportState.onStateGetSettings(state, settings)

    @err_catcher(name=__name__)
    def onStateSettingsLoaded(self, state, settings):
        """Delegate to USD export state module"""
        if self.usdExportState:
            self.usdExportState.onStateSettingsLoaded(state, settings)
